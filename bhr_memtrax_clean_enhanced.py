#!/usr/bin/env python3
"""
BHR MemTrax Clean Enhanced - NO LEAKAGE VERSION
================================================
Building on 0.744 baseline with enhancements but NO cognitive assessments as features
Only using:
- MemTrax performance metrics
- Demographics
- Medical history (non-cognitive)
...
# Medical history QIDs for FEATURES (non-cognitive only!)
MEDICAL_QIDS = {
    'diabetes': ['QID2-1', 'QID2-2'],
    'hypertension': ['QID2-3', 'QID2-4'],
    'heart': ['QID2-5', 'QID2-6'],
    'stroke': ['QID2-7', 'QID2-8'],
    # Removed depression as it's cognitive-related
}


def extract_memtrax_features(memtrax_q):
    """Extract MemTrax features - comprehensive version"""
    features = []
    ...
    for subject, group in memtrax_q.groupby('SubjectCode'):
        ...
        feat['CorrectPCT_cv'] = feat['CorrectPCT_std'] / (feat['CorrectPCT_mean'] + 1e-6)
        feat['CorrectRT_cv'] = feat['CorrectResponsesRT_std'] / (feat['CorrectResponsesRT_mean'] + 1e-6)
...
def add_demographics(df, data_dir):
    ...
    # Additional interactions with cognitive features
    if 'Age_Baseline' in df.columns:
...
def add_medical_history(df, data_dir):
    """Add medical history features (non-cognitive only)"""
    ...
    if med_path.exists():
        try:
            ...
            # Extract medical conditions (non-cognitive only)
...
def main():
    print("\n" + "="*70)
    print("BHR MEMTRAX CLEAN ENHANCED - NO LEAKAGE")
    print("="*70)
    ...
    # Add medical history (non-cognitive)
    print("5. Adding medical history (non-cognitive)...")
    features = add_medical_history(features, DATA_DIR)
    
    # NOTE: NOT adding ECOG/SP-ECOG to avoid leakage!
    print("6. NOT adding cognitive assessments (avoiding leakage)")
    ...
    # Test with optimal feature selection
    ...
    print(f"Clean baseline: {baseline_auc:.4f}")
    print(f"With feature selection: {optimized_auc:.4f}")
    print(f"Best individual model: {best_individual_auc:.4f} ({best_model_name})")
    print(f"\nOverall best (no leakage): {best_auc:.4f}")
    
    if best_auc >= 0.80:
        print(f"\n🎯 LEGITIMATE SUCCESS! Achieved {best_auc:.4f} AUC without leakage!")
    elif best_auc >= 0.78:
        print(f"\n✅ GREAT! Achieved {best_auc:.4f} AUC without leakage!")
    ...
    # Save results
    output = {
        'strategy': 'Clean enhanced baseline - NO cognitive assessments as features',
        'expected_baseline': expected_baseline,
        'clean_baseline': float(baseline_auc),
        ...
        'note': 'No ECOG/SP-ECOG features to avoid leakage'
    }
    
    with open(OUTPUT_DIR / 'clean_enhanced_results.json', 'w') as f:
        json.dump(output, f, indent=2)
    
    print(f"\nResults saved to: {OUTPUT_DIR}/clean_enhanced_results.json")
