...
NOTE: The original best script uses QID1-5, QID1-12, QID1-13, QID1-22, QID1-23
But QID1-17 was identified as problematic in our mismatch analysis.
...
# Experiment 1: Drop QID1-17 (but it's not in original set, so this is a control)
# Let's instead test dropping the most problematic QID from our mismatch analysis
EXPERIMENT_COGNITIVE_QIDS = ['QID1-5', 'QID1-12', 'QID1-13', 'QID1-22', 'QID1-23']  # Same as original
...
def apply_ashford_filter(df, min_acc=0.60):
    ...
    # Processed data doesn't have Status column
...
def main():
    ...
    print("="*60)
    print("EXPERIMENT 1: Test Original vs Modified Cognitive Labels")
    print("="*60)
    print("Testing the original QIDs from best script vs potential improvements")
    print()
    ...
    if "Original" in results and "Experiment" in results:
        ...
        experiment_results = {
            'experiment': 'Test Original vs Modified Cognitive Labels',
        ...
        with open(OUTPUT_DIR / 'experiment1_label_comparison_results.json', 'w') as f:
            json.dump(experiment_results, f, indent=2)
        
        print(f"\nResults saved to: {OUTPUT_DIR / 'experiment1_label_comparison_results.json'}")
        
        if improvement > 0.01:
            print(f"\n🎯 SUCCESS! Modified labels improved AUC by {improvement:.4f}")
        elif improvement > 0:
