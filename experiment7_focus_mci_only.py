#!/usr/bin/env python3
"""
Experiment 7: Focus on MCI Only
==============================

This experiment tests focusing on the most specific cognitive impairment
indicator - Mild Cognitive Impairment (MCI) only - vs the broader set.
...
MCI-ONLY QIDs (1 condition):
- QID1-13: Mild Cognitive Impairment (most specific, least noisy)

Expected Impact: +0.02-0.05 AUC by reducing label noise
"""
...
# MCI-only QIDs - focus on most specific indicator
MCI_ONLY_QIDS = ['QID1-13']  # Only Mild Cognitive Impairment
...
def main():
    ...
    print("="*60)
    print("EXPERIMENT 7: Focus on MCI Only")
    print("="*60)
    print("Testing MCI-only labels vs broader cognitive impairment labels:")
    print("  ORIGINAL QIDs: QID1-5 (Dementia), QID1-12 (Alzheimer's), QID1-13 (MCI), QID1-22 (FTD), QID1-23 (LBD)")
    print("  MCI-ONLY QIDs: QID1-13 (Mild Cognitive Impairment)")
    print("  RATIONALE: MCI is most specific and least noisy cognitive impairment indicator")
    print("Expected Impact: +0.02-0.05 AUC by reducing label noise")
    print()
    ...
    if "Original" in results and "MCI-Only" in results:
        orig_auc = results["Original"]["auc"]
        mci_auc = results["MCI-Only"]["auc"]
        improvement = mci_auc - orig_auc
        
        print(f"Original Labels AUC: {orig_auc:.4f}")
        print(f"MCI-Only Labels AUC: {mci_auc:.4f}")
        ...
        experiment_results = {
            'experiment': 'Focus on MCI Only',
            'original_qids': ORIGINAL_COGNITIVE_QIDS,
            'mci_only_qids': MCI_ONLY_QIDS,
            'original_auc': orig_auc,
            'mci_only_auc': mci_auc,
            'improvement': improvement,
            'relative_improvement_pct': improvement/orig_auc*100,
            'results': results
        }
        
        with open(OUTPUT_DIR / 'experiment7_focus_mci_only_results.json', 'w') as f:
            json.dump(experiment_results, f, indent=2)
        
        print(f"\nResults saved to: {OUTPUT_DIR / 'experiment7_focus_mci_only_results.json'}")
        
        if improvement > 0.02:
            print(f"\n🎯 SUCCESS! MCI-only labels improved AUC by {improvement:.4f}")
        elif improvement > 0:
